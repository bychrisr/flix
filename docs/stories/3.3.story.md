...
## Tasks / Subtasks
- [x] Task 1 (AC: 1) Lesson page renders video player for authorized lessons
  - [x] Implement technical changes required for AC 1
  - [x] Add validation/tests for AC 1
- [x] Task 2 (AC: 2) Adjacent lesson navigation (previous/next) is available when applicable
  - [x] Implement technical changes required for AC 2
  - [x] Add validation/tests for AC 2
- [x] Task 3 (AC: 3) Basic anti-abuse/embed constraints are applied in player integration
...
|------|---------|-------------|--------|
| 2026-02-11 | 1.0 | Story created from 3.3 epic section with architecture context | @sm |
| 2026-02-12 | 1.1 | Playback contract expanded with player metadata, navigation, and embed constraints | @dev |
...
- Extended lesson model to support playback metadata (`videoProvider`, `videoId`).
- Updated lesson create/update contracts with optional video fields and provider validation.
- Expanded playback endpoint response with `player` payload for authorized lessons.
- Added adjacent navigation payload (`previous`/`next`) based on lesson ordering per event.
- Added baseline anti-abuse/embed constraints in playback response contract.
