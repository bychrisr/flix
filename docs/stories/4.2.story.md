...
## Tasks / Subtasks
- [x] Task 1 (AC: 1) Materials list is visible for released lessons
  - [x] Implement technical changes required for AC 1
  - [x] Add validation/tests for AC 1
- [x] Task 2 (AC: 2) Download links resolve to valid files
  - [x] Implement technical changes required for AC 2
  - [x] Add validation/tests for AC 2
- [x] Task 3 (AC: 3) Access respects lesson availability rules
...
| 2026-02-12 | 1.1 | Added learner materials endpoint guarded by access rules with integration test coverage | @dev |
...
- Added learner endpoint `POST /api/public/events/:eventSlug/lessons/:lessonSlug/materials`.
- Endpoint reuses release-window and private-event checks before returning lesson materials.
- Materials payload exposes valid download links created by admin upload flow.
- Added integration coverage for released vs locked lesson access to materials.
