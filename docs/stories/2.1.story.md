...
## Tasks / Subtasks
- [x] Task 1 (AC: 1) Admin can create, update, list, and delete events
  - [x] Implement technical changes required for AC 1
  - [x] Add validation/tests for AC 1
- [x] Task 2 (AC: 2) Event uniqueness constraints (e.g., slug) are enforced
  - [x] Implement technical changes required for AC 2
  - [x] Add validation/tests for AC 2
- [x] Task 3 (AC: 3) Event activation state is represented and editable
  - [x] Implement technical changes required for AC 3
  - [x] Add validation/tests for AC 3
- [x] Task 4 (AC: 4) Event visibility (`public` or `private`) is represented and editable
...
| 2026-02-12 | 1.1 | Event CRUD and listing delivered with integration coverage | @dev |
...
- Implemented admin-protected event CRUD routes: list, create, update, delete.
- Added event domain service with in-memory repository for MVP lifecycle.
- Enforced slug uniqueness for create and update operations.
- Added editable `isActive` and `visibility` (`public`/`private`) attributes.
- Added request validation for create/update payloads with standardized validation errors.
