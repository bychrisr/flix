...
## Tasks / Subtasks
- [x] Task 1 (AC: 1) Quiz CRUD supports multiple questions and options
  - [x] Implement technical changes required for AC 1
  - [x] Add validation/tests for AC 1
- [x] Task 2 (AC: 2) Validation enforces at least one correct option per question
  - [x] Implement technical changes required for AC 2
  - [x] Add validation/tests for AC 2
- [x] Task 3 (AC: 3) Quiz structure persists consistently in database relations
...
|------|---------|-------------|--------|
| 2026-02-11 | 1.0 | Story created from 4.3 epic section with architecture context | @sm |
| 2026-02-12 | 1.1 | Implemented quiz CRUD with nested questions/options and validation for correct answers | @dev |
...
- Added `quiz-service` with quiz/question/option graph persistence in in-memory relations.
- Implemented admin quiz CRUD endpoints:
  - `POST /api/quizzes`
  - `GET /api/quizzes/:quizId`
  - `PUT /api/quizzes/:quizId`
  - `DELETE /api/quizzes/:quizId`
- Enforced business validation requiring at least one correct option per question.
- Added integration tests covering CRUD lifecycle and validation failure scenarios.

### File List
- services/api/src/services/quiz-service.js
- services/api/src/routes/quizzes.js
